...
describe("GET /1", function () {
  ...
  test("It should return 404 doesnt exist", async function () {
    const response = await request(app).get("/invoices/2462363");
...
describe("PUT /", function () {
  ...
  test("It should return 404 and doesnt exist", async function () {
    const response = await request(app)
        .put("/invoices/56262773")
        .send({amt: 1000});

    expect(response.status).toEqual(404);
  });

  test("It should return 500 and missing data", async function () {
    const response = await request(app)
        .put("/invoices/1")
        .send({});
